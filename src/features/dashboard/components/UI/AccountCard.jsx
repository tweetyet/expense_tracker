...
const AccountCard = ({
  ...
  return (
    ...
    //   </div>
    // </div>
    <div className="flex items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm hover:shadow-md transition-all duration-200">
  ...
  <div className="flex items-center gap-3 min-w-0">
    ...
    <div
      ...
      ${bgColor || "bg-amber-200"} shrink-0`}
    >
      {Icon ? (
        <Icon size={16} className={iconColor} />
      ) : (
        <span className="text-sm text-[#43302E]">IC</span>
      )}
    </div>

    <div className="min-w-0">
      <p className="font-medium text-[#43302E] truncate">
        {name}
      </p>
      <p className="text-sm text-[#43302E]/70 truncate">
        {description}
      </p>
    </div>
  </div>
  ...
  {actionLabel && (
    <span
      className={`text-xs px-2 py-1 font-semibold rounded-lg whitespace-nowrap ${actionColor}`}
    >
      {actionLabel}
    </span>
  )}
